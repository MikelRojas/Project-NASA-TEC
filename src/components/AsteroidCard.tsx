  ...
  return (
    <div className="card asteroid-card text-center">
      ...
      <div className="card-body">
        ...
        <p className="card-text"><strong>{t("asteroid.Magnitude")}:</strong> {asteroid.absolute_magnitude_h}</p>
        <p className="card-text"><strong>{t("asteroid.Diameter")}:</strong> {asteroid.estimated_diameter.kilometers.estimated_diameter_min.toFixed(2)} - {asteroid.estimated_diameter.kilometers.estimated_diameter_max.toFixed(2)} km</p>
        <p className="card-text"><strong>{t("asteroid.Hazardous")}:</strong> {asteroid.is_potentially_hazardous_asteroid ? 'Yes' : 'No'}</p>
        {asteroid.close_approach_data.map((approach, index) => (
          <div key={index}>
            <p className="card-text"><strong>{t("asteroid.Close")}</strong> {approach.close_approach_date}</p>
            <p className="card-text"><strong>{t("asteroid.Velocity")}:</strong> {parseFloat(approach.relative_velocity.kilometers_per_hour).toFixed(2)} km/h</p>
            <p className="card-text"><strong>{t("asteroid.Distance:")}</strong> {parseFloat(approach.miss_distance.kilometers).toFixed(2)} km</p>
          </div>
        ))}
